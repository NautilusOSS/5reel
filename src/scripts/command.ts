...
program
  .name("5reel")
  .description(
    "5Reel Slot Machine CLI - Deploy and interact with Algorand smart contracts"
  )
  .version("1.0.0");
...
program
  ...
  .action(() => {
    ...
    console.log("=================");
    console.log(
      `Algod Server: ${options.algodServer || "Using default for " + network}`
    );
    console.log(
      `Algod Port: ${options.algodPort || "Using default for " + network}`
    );
    console.log(
      `Indexer Server: ${
        options.indexerServer || "Using default for " + network
      }`
    );
    console.log(
      `Indexer Port: ${options.indexerPort || "Using default for " + network}`
    );
    console.log("");
...
program
  ...
  .action(() => {
    ...
    console.log(
      "  -n, --network <string>     Specify network (devnet, testnet, mainnet)"
    );
    console.log(
      "  --algod-server <string>    Override Algorand node server URL"
    );
    console.log("  --algod-port <number>      Override Algorand node port");
    console.log(
      "  --indexer-server <string>  Override Algorand indexer server URL"
    );
    ...
    console.log(
      "  -s, --simulate             Simulate transactions without sending"
    );
    console.log(
    ...
    console.log(
      "  config                     Show current configuration and network settings"
    );
    ...
    console.log(
      "  ybt-withdraw               Withdraw from yield bearing token"
    );
    ...
    console.log(
      "  ybt-revoke-yield-bearing-source  Revoke yield bearing source"
    );
    ...
    console.log(
      '  5reel -n testnet -d deploy -t SlotMachine --name "MySlotMachine"'
    );
    console.log("");
...
const getCurrentNetworkConfig = () => {
  ...
  const config = {
    server: globalOptions.algodServer || (globalThis as any).ALGO_SERVER || ALGO_SERVER,
    port: globalOptions.algodPort || (globalThis as any).ALGO_PORT || ALGO_PORT,
    indexerServer: globalOptions.indexerServer || (globalThis as any).ALGO_INDEXER_SERVER || ALGO_INDEXER_SERVER,
    indexerPort: globalOptions.indexerPort || (globalThis as any).ALGO_INDEXER_PORT || ALGO_INDEXER_PORT,
...
export const spin = async (options: SpinOptions) => {
  ...
  const paymentAmount =
    options.betAmount * (options.maxPaylineIndex + 1) + spin_cost; //+
...
export const simulateGridPaylineMatch = (
  grid: string,
  payline: number[],
  payoutMultipliers?: { [key: string]: number }
) => {
  ...
  for (let i = 0; i < paylineSymbols.length; i++) {
    if (paylineSymbols[i] === "_") continue; // Skip wildcards
  ...
  for (const [symbol, count] of Object.entries(symbolCounts)) {
    if (count < 3) continue; // Need at least 3 matches for payout
...
export const simulateGridPaylineSymbols = (grid: string, payline: number[]) => {
  const gridArray = grid.split("");
  const paylineSymbols = payline.map(
    (rowIndex, colIndex) => gridArray[colIndex * 3 + rowIndex]
  );
  const paylineSymbolsArray = paylineSymbols.map(
    (symbol) => new Uint8Array(Buffer.from(symbol))
  );
  const paylineSymbolsString = paylineSymbolsArray.map((symbol) =>
...
interface GetGridPaylineSymbolsOptions {
  appId: number;
  grid: string;
  paylineIndex: number;
  addr: string;
...
export const getGridPaylineSymbols: any = async (
  options: GetGridPaylineSymbolsOptions
) => {
  const addr = options.addr || addressses.deployer;
  const sk = options.sk || sks.deployer;
  const acc = { addr, sk };
  const ci = makeContract(options.appId, SlotMachineAppSpec, acc);
  const getGridPaylineSymbolsR = await ci.get_grid_payline_symbols(
    options.grid,
    options.paylineIndex
  );
  if (options.debug) {
...
interface YBTGetMaxWithdrawableAmountOptions {
  appId: number;
  address: string;
  addr: string;
  sk: any;
...
export const ybtGetMaxWithdrawableAmount: any = async (
  options: YBTGetMaxWithdrawableAmountOptions
) => {
  const addr = options.addr || addressses.deployer;
  const sk = options.sk || sks.deployer;
  const acc = { addr, sk };
  const ci = makeContract(options.appId, YieldBearingTokenAppSpec, acc);
  ci.setFee(7000);
  const getMaxWithdrawableAmountR = await ci.get_max_withdrawable_amount(
    options.address
  );
  if (options.debug) {
...
export const getSlot: any = async (options: GetSlotOptions) => {
  const addr = options.addr || addressses.deployer;
  const sk = options.sk || sks.deployer;
  const acc = { addr, sk };
  const ci = makeContract(options.appId, SlotMachineAppSpec, acc);
...
program
  ...
  .action(async (options: GetBalancesOptions) => {
...
export const syncBalance: any = async (options: SyncBalanceOptions) => {
  const addr = options.addr || addressses.deployer;
  const sk = options.sk || sks.deployer;
  const acc = { addr, sk };
  const ci = makeContract(options.appId, BankManagerAppSpec, acc);
